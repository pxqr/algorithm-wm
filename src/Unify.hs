...
data UFailure = LitMismatchE Name Name UTrace
              | OccChkFailE  Name Ty   UTrace
              | UFailureE    Ty   Ty   UTrace
              | UStrMsgE     String

instance Error UFailure where
...
instance Pretty UFailure where
...
type UResult = Either UFailure
type UnifierM t m = StateT (Subst t) m

type Unifier t = UnifierM t UResult
